...
The reviewed 0.2 codebase has a deliberately narrow read-only design and no plugin
...
### SEC-002 — Unnecessary session identifier exposure (Low)

The initial history response returned the internal session ID for UI keys. The field was
removed; frontends use non-persistent render keys.
